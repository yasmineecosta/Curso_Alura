...
#include <string>
#include <map>
#include <vector>   //vetor de alocacao dinamica
#include <fstream>
#include <ctime>
#include <cstdlib>
using namespace std;
...
vector<string> le_arquivo(){
    ifstream arquivo;       //leitura de arquivos
    ...
    if(arquivo.is_open()){
        ...
        exit(0);    //para a execucao na hora
    }
}
...
void salva_arquivo(vector<string> nova_lista){
    ofstream arquivo;   // output, f - file, stream - fluxo de dados. Lida com a escrita de valores no arquivo
                //escrever em arquivos
    arquivo.open("palavras.txt");
    if(arquivo.is_open()){
        arquivo << nova_lista.size() << endl;

        for(string palavra : nova_lista){   //C++ 11
